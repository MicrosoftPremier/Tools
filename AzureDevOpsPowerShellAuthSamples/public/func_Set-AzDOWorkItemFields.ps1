function Set-AzDOWorkItemFields {
    ...
    param (
        ...
        [Parameter(Mandatory = $true, HelpMessage = "The field/value pairs to set.")]
        [string[]] $Fields,
    ...
    # Get work item prior to update
    $WorkItem = Get-AzDOWorkItem -Organization $Organization -Project $Project -WorkItemId $WorkItemId @AuthParams
    ...
    foreach ($Field in $Fields) {
        ...
        $Body += @{
            op = "add"
            path = "/fields/$($FieldTokens[0])"
            value = $FieldTokens[1]
        }
    }
    $Headers["Content-Type"] = "application/json-patch+json"
    $ApiResult = Invoke-RestMethod -Uri "$BaseUri/$Project/_apis/wit/workitems/$($WorkItemId)?api-version=$ApiVersion" -Method Patch -Body ($Body | ConvertTo-Json -Depth 10) -Headers $Headers -SkipHttpErrorCheck -StatusCodeVariable ApiStatusCode
    if ($ApiStatusCode -ne 200) {
        if ($ApiResult.message) {
            Write-Host $ApiResult.message -ForegroundColor Red
        } else {
            Write-Host "Unable to update work item!" -ForegroundColor Red
        }
        exit 1
    }
